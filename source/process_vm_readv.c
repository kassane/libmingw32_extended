...
ssize_t process_vm_readv(pid_t __pid, const struct iovec *__lvec,
  ...
  size_t offRvec = 0;
  size_t writeLvec = 0;
  HANDLE hProcess = OpenProcess(PROCESS_VM_READ, FALSE, __pid);
  ...
  while (iLvec != __liovcnt && iRvec != __riovcnt && writeLvec != 0) {
    ReadProcessMemory(
        ...
        (void *)((size_t)__lvec[iLvec].iov_base + offLvec), writeLvec, NULL);
    offLvec += writeLvec;
    offRvec += writeLvec;
