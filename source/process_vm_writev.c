...
ssize_t process_vm_writev(pid_t __pid, const struct iovec *__lvec,
  ...
  size_t writeRvec = 0;
  HANDLE hProcess =
      OpenProcess(PROCESS_VM_WRITE | PROCESS_VM_OPERATION, FALSE, __pid);
  ...
  while (iLvec != __liovcnt && iRvec != __riovcnt && writeRvec != 0) {
    WriteProcessMemory(
        hProcess, (void *)((size_t)__rvec[iRvec].iov_base + offRvec),
        (void *)((size_t)__lvec[iLvec].iov_base + offLvec), writeRvec, NULL);
    offLvec += writeRvec;
    offRvec += writeRvec;
